...
<a id="write" href="mailWrite?mem_id=${member.mem_id}">메일쓰기</a>
	<a id="write" href="mailtome?mem_id=${member.mem_id}">내게쓰기</a>
 		<ul id="mailbox1">
			...
			<li><a class="checkmailbox" href="sen_mailList?mem_id=${member.mem_id }">보낸메일함</a></li> 
			<li><a href="del_mailList?mem_id=${member.mem_id }">삭제된메일함</a></li>
		</ul>
</div>
<div id="table">
	<div class="row">
			<span class="cell col1">받는 사람</span>
 ...
 <div class="currentpage">
	 <c:if test="${pg.startPage > pg.pageBlock }">
		<a href="sen_mailList?currentPage=${pg.startPage-pg.pageBlock}&mem_id=${member.mem_id}">[이전]</a>
	</c:if>
	<c:forEach var="i" begin="${pg.startPage }" end="${pg.endPage }">
		<a class="currentpage1" href="sen_mailList?currentPage=${i}&mem_id=${member.mem_id}">[${i}]</a>
	</c:forEach>
	<c:if test="${pg.endPage < pg.totalPage }">
		<a href="sen_mailList?currentPage=${pg.startPage+pg.pageBlock}&mem_id=${member.mem_id}">[다음]</a>
	</c:if>
</div>
		</div>
	</div>
