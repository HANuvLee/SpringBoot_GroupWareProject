                ...
                success:function(data){
                    ...
                    if(data.length > 0){
                        ...
                        for(var i in data){
                            ...
                            // 자신이 구매자 입장일 때
                            if(data[i].mem_id == "${member.sen_message_id}"){
                                // 현재 판매자가 로그인 상태 일 때
                                if(loginList.indexOf(data[i].you_id) != -1){
                                    $div = $("<div class='chatList_box enterRoomList' onclick='enterRoom(this);'>").attr("no",data[i].room_no).attr("sen_message_id",data[i].you_id);
                                }
                                // 현재 판매자가 로그아웃 상태 일 때
                                else{
                                    $div = $("<div class='chatList_box2 enterRoomList' onclick='enterRoom(this);'>").attr("no",data[i].room_no).attr("sen_message_id",data[i].you_id);
                                }
                                $img = $("<img class='profile_img'>").attr("src", "resources/masterImg/" + data[i].masterPic);
                                $divs = $("<div class='userNameId'>").text(data[i].you_id);
                            }
                            // 자신이 판매자 입장일 때
                            else{                        
                                // 현재 구매자가 로그인 상태 일 때
                                if(loginList.indexOf(data[i].userEmail) != -1){
                                    $div = $("<div class='chatList_box enterRoomList' onclick='enterRoom(this);'>").attr("no",data[i].room_no).attr("sen_message_id",data[i].mem_id);
                                }
                                // 현재 구매자가 로그아웃 상태 일 때
                                else{
                                    $div = $("<div class='chatList_box2 enterRoomList' onclick='enterRoom(this);'>").attr("no",data[i].room_no).attr("sen_message_id",data[i].mem_id);
                                }                                
                                $img = $("<img class='profile_img'>").attr("src", "resources/img/" + data[i].userPic);
                                $divs = $("<div class='userNameId'>").text(data[i].mem_name);
                            }
                            
                            // 읽지 않은 메세지가 0이 아닐 때
                            if(data[i].unread_count != 0){
                                $span = $("<span class='notRead'>").text(data[i].unread_count);
                            }else{
                                $span = $("<span>");
                            }
    ...
    <script>
        ...
        // 웹소켓
         ...
         function onOpen() {
             ...
             const data = {
                    "room_no" : room_no,
                    "sen_message_name" : "${ member.sen_message_name }",
                    "sen_message_id" : "${ member.sen_message_id }",
        ...
        function sendMessage(message_content){
         ...
         function onMessage(evt) {
            ...
            const data = {
             ...
             if(data.sen_message_id != "${ member.sen_message_id }"){
                CheckLR(data);
             }
        }
        ...
        function CheckLR(data) {
            ...
            const LR = (data.sen_message_id != "${ member.sen_message_id }") ? "left" : "right";
             // 메세지 추가
            appendMessageTag(LR, data.sen_message_id, data.message_content, data.sen_message_name);
        }
         
        // * 3 메세지 태그 append
        function appendMessageTag(LR_className, sen_message_id, message_content, sen_message_name) {
             
            const chatLi = createMessageTag(LR_className, sen_message_id, message_content, sen_message_name);
        ...
        // * 4 메세지 태그 생성
        function createMessageTag(LR_className, sen_message_id, message_content, sen_message_name) {
             ...
             chatLi.find('.sender span').text(sen_message_name);      // 이름 추가
             chatLi.find('.message span').text(message_content); // 메세지 추가
