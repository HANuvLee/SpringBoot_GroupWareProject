		...
		<div id="sc_cont_ul">
			...
			<a id="write" href="mailWrite?mem_id=${member.mem_id}">메일쓰기</a>
				<a id="write" href="mailtome?mem_id=${member.mem_id}">내게쓰기</a>
			 		<ul id="mailbox1">
						<li><a class="checkmailbox" href="mailList?mem_id=${member.mem_id }">전체메일함</a></li>
			...
			<div id="table">
			 ...
			 <div class="currentpage">
				 <c:if test="${pg.startPage > pg.pageBlock }">
					<a href="mailList?currentPage=${pg.startPage-pg.pageBlock}&mem_id=${member.mem_id}">[이전]</a>
				</c:if>
				<c:forEach var="i" begin="${pg.startPage }" end="${pg.endPage }">	
						<a class="currentpage1" href="mailList?currentPage=${i}&mem_id=${member.mem_id}">[${i}]</a>
				</c:forEach>
				<c:if test="${pg.endPage < pg.totalPage }">
					<a  href="mailList?currentPage=${pg.startPage+pg.pageBlock}&mem_id=${member.mem_id}">[다음]</a>
				</c:if>
			</div>
		</div>
	</div>
