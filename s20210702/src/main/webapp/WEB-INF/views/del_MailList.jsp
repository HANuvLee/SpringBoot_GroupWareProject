...
<script type="text/javascript">
	function restore(){
		...
		$.ajax({
			url: "http://localhost:8282/s20210702/allrestoreMail",
			type: 'get',
			data: {"delArray": chkArray},
			dataType:"json",
			async:false,
			success: function(date) {
				alert(date + "개의 메일이 복구되었습니다.")
...
<div class="mailbotton">
	...
	<button id="write" type="button" onclick="restore()">복구</button>
	<button id="write" type="button" onclick="deletes()">영구삭제</button>
</div>
<div id="mailbox">
<a id="write" href="mailWrite?mem_id=${member.mem_id}">메일쓰기</a>
	<a id="write" href="mailtome?mem_id=${member.mem_id}">내게쓰기</a>
 		<ul id="mailbox1">
			...
			<li><a class="checkmailbox" href="del_mailList?mem_id=${member.mem_id }">삭제된메일함</a></li>
		</ul>
</div>
<div id="table">
<div class="row">
 	...
 	<c:forEach var="mail" items="${listMail }">
		<div class="row">
			...
			<span class="cell col2"><a class="readed" href="mailDetail?mail_no=${mail.mail_no }&mail_receiver=${mail.mail_receiver}">[${mail.chk}]${mail.mail_title}</a></span> 
...
<div class="currentpage">
	...
	<c:forEach var="i" begin="${pg.startPage }" end="${pg.endPage }">
		<a class="currentpage1" href="del_mailList?currentPage=${i}&mem_id=${member.mem_id}">[${i}]</a>
	</c:forEach>
